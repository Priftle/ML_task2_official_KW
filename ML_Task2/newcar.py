...
class Car:
    ...
    """ 2:
    This function is a display function that draws objects onto the screen.
    The first part of the function displays and positions the car sprite using the blit function that copies a
    specified image onto the surface of the screen. The parameter self.rotated_sprite defines the image source
    and the self.position parameter defines the coordinates the image shall be placed.
    (defined as 830, 920 in the prior function)
    The second half displays the radars attached to the car as defined by the following function.
    """
    ...
    """ 3:
    This function defines each of the radars' position and how they will be displayed as lines with small 
    circles at the ends of them.
    """
    ...
    """ 4:
    This function checks for any collisions with the artificaialy inteligent car 'self'.
    A collision is detected by checking that the car is in contact with the colour of the border in which case 
    the car is terminated by redefining self.alive as false, otherwise if no contact is detected, the car remains alive.
    """
    ...
    """ 5:
    This function begins by resetting length to 0, then it checks along each point of the radar using a while loop to detect 
    where the border is. The border is detected by checking if a point on the radar makes contact with the border colour, 
    when a border is detected or the radar reaches 300 length, the loop is ended. The distance from the centre of the car to the border 
    is then calculated and appended to the ‘self.radar’ list to be used in the ‘get_data’ function.
    """
    ...
    """ 6:
    This function updates a car's position coordinates and calculates the consequential changes to movement and
    the new corners and centre of the rectangular car.self.speed is redefined to 20 which will be altered in order
    for the car to speed up or slow down in the 'run_simulation' function. The car's x (self.position[0]) and y (self.position[1])
    positions are updated with minimums and maximums. The car's distance is determined by its speed, and the time is increased by 1.
    The new x and y centres are calculated and assigned to 'self.center'. The four corner of the rectangular car used to detect
    collisions are calculated and assigned to 'self.corners'. Any collisions are checked and the radars are cleared to return new
    values when self.radar() is called below.
    """
    ...
    """ 7:
    This function returns the distance of each of the 5 radars to the border.
    A loop is created for each of the radars to get the distance value of the radars to the boarders
    and assign it to the list 'return_values'. This list is then returned to the get_data function which will be used to
    dictate the action a car will take: if it will turn left, right, speed up or slow down.
    """
    ...
    """ 8:
    This function is the basic alive function that returns the boolean value of self.alive, communicating
    weather each car has crashed or not. This value will be used later on to determine whether a car's fitness
    shall increase if it is alive or shall remain the same if it is not alive.
    """
    ...
    """ 9:
    This function calculates a reward for each car based on the greater length of distance that has been travelled,
    This returned reward scale is used to increase the fitness of the car within the 'run_simulation' function later on.
    """
    ...
    """ 10:
    This function rotates the rectangular image of the car so the sprite follows the angle direction that the car is turning.
    The centre is adjusted to align with the image and the 'rotated_image' value is returned to be used in the previous 'update'
    function.
    """
...
3.  Creating Neural Networks and Cars:
    ...
    A neural network is created using the neat.nn.FeedForwardNetwork.create(g, config) method. 
    The genome g and configuration config are used to create the network.
